...
let result3 = document.querySelector('#resultT3');
...
enter3.addEventListener('input', (ev) => {
    if (isNaN(+ev.target.value) || ev.target.value === '' || ev.target.value < 2)
        return;
...
function sum(arrN1, arrN2) {
    ...
    for (let i = 0; i < arrN1.length; i++) {
        ...
        if (i === arrN1.length - 1 && remainder === 1)
            resultArray.unshift(1);
    }
    return resultArray;
}
function multiplicate(arrN1, arrN2) {
    let resultArray = [];
    let tempArray = [];
    let remainder = 0;
    arrN1 = arrN1.slice(0).reverse();
