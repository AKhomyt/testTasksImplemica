...
let result2: any = document.querySelector('#resultT2')!
...
function calculate (path: string, array?: Array<string>) {
  // path => cityA-cityB
  const calcArray: any = array || []
...
function cost (pathSting: string) {
...
function sortByPricecoulck (cityString: string) {
  const calcArray: Array<any> = []
...
function trFunc (path: string, cost: number) {
